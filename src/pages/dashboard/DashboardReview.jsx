  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex mb-3">
        <button
          className={`px-8 py-2 rounded-md mr-4 ${activeTab === "JOB_OPENING" ? "bg-[#43312A] text-white" : "bg-[#E8C5A5] text-gray-800"}`}
          onClick={() => handleTabClick("JOB_OPENING")}
        >
          산책인 모집글
        </button>
        <button
          className={`px-8 py-2 rounded-md ${activeTab === "JOB_SEARCH" ? "bg-[#43312A] text-white" : "bg-[#E8C5A5] text-gray-800"}`}
        ...
        {reviews.length > 0 ? (
          <>
            {reviews.slice(0, visibleReviews).map((review, index) => (
              <div key={review.id} className={`p-4 bg-white ${index !== 0 ? "border-t border-gray-300" : ""}`}>
            ...
            {!showNoMoreReviews && (
              <div className="flex justify-end mt-4 p-4 border-t border-gray-300">
                <button className="px-4 py-2 bg-[#E8C5A5] text-gray-800 rounded-md" onClick={handleLoadMore}>
                  더보기
                </button>
              </div>
            )}
