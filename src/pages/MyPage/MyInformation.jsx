...
const schema = yup.object().shape({
  ...
  tel: yup.string().matches(/^\d+$/, '전화번호는 숫자로만 입력해주세요.').required('전화번호는 필수 입력 사항입니다.'),
  address: yup.string().required('주소는 필수 입력 사항입니다.'),
  birth: yup.date().required('생년월일은 필수 입력 사항입니다.'),
  // password: yup.string().min(8, '비밀번호는 최소 8자 이상이어야 합니다.').required('비밀번호는 필수 입력 사항입니다.'),
});
...
const MyInformation = () => {
    ...
    const onSubmit = (data) => {
      console.log(data); // 여기서 데이터를 처리하거나 API 호출 등을 수행할 수 있습니다.
      MySwal.fire({
        title: "회원 정보를 저장했습니다",
        icon: "success",
  ...
  return (
      <div className="max-h-screen overflow-y-auto p-4">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <button
            type="submit"
            className={`px-4 py-2 bg-[#43312A] text-white rounded-md hover:bg-yellow-700 ${Object.keys(errors).length !== 0 && 'opacity-50 cursor-not-allowed'}`}
